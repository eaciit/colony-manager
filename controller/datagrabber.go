...
func (d *DataGrabberController) AutoGenerateDataSources(payload *colonycore.DataGrabberWizardPayload, formatTime string) ([]*colonycore.DataGrabber, error) {
	...
	for key, each := range payload.Transformations {
		if isNosql {
			...
			if !toolkit.HasMember(collObjectNames, each.TableDestination) {
				...
				err = connDest.NewQuery().Insert().From(each.TableSource).Exec(toolkit.M{"data": data}) //.Save(each.TableSource, metaToSave, nil)
				if err != nil {
					return result, err
				}
