...
func (d *DataGrabberController) SaveDataGrabberWizard(r *knot.WebContext) interface{} {
	...
	success, message := d.AutoGenerateDataSources(payload, formatTime)
	_ = success
	if err != nil {
		helper.CreateResult(false, nil, message)
	}

	return helper.CreateResult(true, payload, "")
}

func (d *DataGrabberController) AutoGenerateDataSources(payload *colonycore.DataGrabberWizardPayload, formatTime string) (bool, string) {
	fmt.Println("*** ", payload.Transformations)
	...
	for key, each := range payload.Transformations {
		...
		for i := 0; i < 2; i++ {
			...
			if payload.Prefix != "" {
				cdsID = payload.Prefix + "DS_" + strconv.Itoa(i) + strconv.Itoa(key) + "_W" + formatTime
			} else {
				cdsID = "DS_" + strconv.Itoa(i) + strconv.Itoa(key) + "_W" + formatTime
			}
		...
		owiz.ID = "GRAB_TEST_" + strconv.Itoa(key) + formatTime
