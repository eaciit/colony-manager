...
func (w *WebGrabberController) PrepareGrabConfigForDoc(o *colonycore.WebGrabber) (*sedotan.GrabService, error) {
	...
	if o.GrabConfiguration.Has("doctype") {
		...
		gService.ServGetData, err = sedotan.NewGetDatabase(ci.Host, o.GrabConfiguration["doctype"].(string), &ci)
		if err != nil {
			return nil, err
		}
	}
	...
	gService.Log = logConf
	gService.ServGetData.CollectionSettings = make(map[string]*sedotan.CollectionSetting)
	gService.DestDbox = make(map[string]*sedotan.DestInfo)

	dataSettings := sedotan.CollectionSetting{}
	destinationInfo := sedotan.DestInfo{}

	dataSettings.Collection = o.DataSetting.RowSelector

	for _, columnSet := range o.DataSetting.ColumnSettings {
		dataSettings.SelectColumn = append(dataSettings.SelectColumn, &sedotan.GrabColumn{Alias: columnSet.Alias, Selector: columnSet.Selector})
	}
