...
func (a *ApplicationController) ReadDirectory(r *knot.WebContext) interface{} {
	...
	var ret []TreeSourceUi
	path, err := os.Getwd()
	// var dir = filepath.Join(path, "..", "..", "..")
	var dir = filepath.Join(path, "..", "colony-app", "apps")
	fmt.Println("==============", dir)
...
func (a *ApplicationController) GenNewFile(r *knot.WebContext) interface{} {
	...
	fullpathfile := filepath.Join(path, value)
	// fmt.Println(fullpathfile)
	d1 := []byte("")
    err = ioutil.WriteFile(fullpathfile, d1, 0644)
    check(err)
