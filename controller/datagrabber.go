...
func (d *DataGrabberController) SaveDataGrabberWizard(r *knot.WebContext) interface{} {
	...
	d.AutoGenerateDataSources(payload)

	return helper.CreateResult(true, payload, "")
}

func (d *DataGrabberController) AutoGenerateDataSources(pay colonycore.DataGrabberWizardPayload) {
	payload := pay
	...
	for key, each := range payload.Transformations {
		for i := 0; i < 2; i++ {
			...
			cds.ID = "DSW_" + strconv.Itoa(i) + "_200_" + strconv.Itoa(key)
